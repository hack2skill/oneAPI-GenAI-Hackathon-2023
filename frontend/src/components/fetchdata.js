...
const ServiceCard = ({ item, tokens }) => {
	...
	const contentComponent = () => {
		return (
			<div>
				...
				<div className="z-10000 popup fixed top-0 left-0 w-full h-full flex items-center justify-center bg-black bg-opacity-50">
	...
	return (
		<div
			className="card flex flex-col justify-between p-5 m-2 border border-gray-300 rounded-2xl transition-shadow duration-200 relative"
			style={{ minHeight: "100px" }}
		>
			{/* Emphasized Year Range */}

			<div>
				<h5 className="card-title text-lg font-bold text-gray-900 mb-2 absolute top-5 left-5">
			...
			<div className="content mt-10">
				...
				{request_status === "Delivered" || request_status === "Extracting" ? (
...
const RightSide = ({ tokens, userId, showHistory, setShowHistory }) => {
	...
	return (
		<div className="right-side p-4">
			...
			{showHistory && (
				// If showHistory is true, add a floating button on the bottom right
				<button
					onClick={() => setShowHistory(false)}
					className="fixed bottom-4 right-4 z-10 bg-red-500 border border-red-600 text-white py-2 px-4 rounded hover:bg-red-700 focus:outline-none focus:ring focus:ring-red-200"
				>
					Hide history
				</button>
			)}
